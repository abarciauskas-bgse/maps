...
class Tile {
  ...
  getZarrData(chunk, key, resolve) {
    console.log('calling this._loader')
    console.log(chunk)
    console.log(key)
    return this._loader(chunk, 0, 0, 0, (err, data) => {
      console.log('calling this._loader')
      this.chunkedData[key] = data
      resolve(true)
    })
  ...
  async loadChunks(chunks, zoom) {
    console.log('calling loadChunks')
    this.loading = true
    this._resetReady()
        ...
        (chunk) =>
          new Promise((resolve) => {
            ...
            if (this.chunkedData[key]) {
              ...
              console.log('calling this.getZarr Data')
